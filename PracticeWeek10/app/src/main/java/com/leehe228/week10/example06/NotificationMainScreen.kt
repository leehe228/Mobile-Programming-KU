...
fun formatDateTime(
    epochMillis: Long, hour: Int, minute: Int
): String {
    val dateTime =
...
fun NotificationMainScreen(modifier: Modifier = Modifier) {
    ...
    val timePickerState = rememberTimePickerState(
        initialHour = LocalDateTime.now().hour, initialMinute = LocalDateTime.now().minute
    )
    ...
    pickerMode?.let { mode ->
        when (mode) {
            PickerMode.Date -> {
                DatePickerDialog(onDismissRequest = { pickerMode = null }, confirmButton = {
                    TextButton(onClick = {
                        pickedDateMillis =
            ...
            PickerMode.Time -> {
                TimePickerDialog(onDismiss = { pickerMode = null }, onConfirm = {
                    val dateMillis = pickedDateMillis ?: Instant.now().toEpochMilli()
